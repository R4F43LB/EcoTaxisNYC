...
bucket_name = 'data_alquiler_gran_volumen'  # Bucket 
...
def process_uberLyft_G1(event, context):
    """Se activa cada vez que se sube un archivo al bucket 'data_alquiler_gran_volumen'."""
    ...
    # Cargamos un dataframe con el dataset de taxis Uber y Lyft en New York
    uberLyft = pd.read_parquet(temp_file.name)
    ...
    job = client_bigquery.load_table_from_dataframe(uberLyft, table_ref, job_config=job_config)
...
# rafael@i5G9:~$ gcloud functions deploy process_uberLyft_G1 --runtime python39 --trigger-bucket data_alquiler_gran_volumen --entry-point process_uberLyft_G1 --timeout 3000s --memory 8GB
